...
Test(sfmm_student_suite, coalesce_middle, .timeout = TEST_TIMEOUT) {
	size_t sz_x = 1000, sz_y = 1000;
	void *x = sf_malloc(sz_x);
	void *y = sf_malloc(sz_y);

	cr_assert_not_null(x, "y is NULL!");
	cr_assert_not_null(y, "y is NULL!");
	sf_free(x);
	sf_free(y);
...
Test(sfmm_student_suite, free_large_block, .timeout = TEST_TIMEOUT) {
	sf_errno = 0;
	void *x = sf_malloc(32704);
	cr_assert_not_null(x, "x is NULL!");
	...
	assert_quick_list_block_count(0, 0);
	assert_free_block_count(0, 1);
	assert_free_block_count(32720, 1);
	cr_assert(sf_errno == 0, "sf_errno is not 0!");
}

Test(sfmm_student_suite, malloc_quick_list, .timeout = TEST_TIMEOUT) {
	void *x = sf_malloc(32);
	void *y = sf_malloc(48);
	cr_assert_not_null(x, "x is NULL!");
...
Test(sfmm_student_suite, realloc_new_page, .timeout = TEST_TIMEOUT) {
	void *x = sf_malloc(32);
...
Test(sfmm_student_suite, all_functions, .timeout = TEST_TIMEOUT) {
	void *x = sf_malloc(1000);

	cr_assert_not_null(x, "x is NULL!");

	x = sf_realloc(x, 16000);
